...
- **Geometry** (`src/components/edge-geometry.ts`): `buildEdgeGeometry()` computes the SVG path `d` for a given `routing` (`straight` | `smooth-step` | `orthogonal` | `curved`). The orthogonal/smooth-step router builds each port's lead-out along its own side normal (`sideNormal()`) so a line always approaches a port in that port's natural direction — it does not just dogleg based on the source side. This same builder produces the path for both the live edge and the "ghost" preview while dragging a new connection, so a completed connection never jumps. When an edge names no ports of its own, `facingSides()` picks the pair on the dominant axis between the two centres, rather than the fixed `right`→`left` this used to assume: a hardcoded pair is right for a left-to-right diagram only, and in a top-to-bottom one it made every line leave the source's right edge, double back around the source and enter the target's left — measured on an imported flowchart, 20 of 11 edges passed back through their own endpoint, and 0 after. Only edges with no explicit sides are affected; a line the user drew port-to-port always carries `fromSide`/`toSide` from `onConnect`.
...
**The layout has no user-facing command.** It ran as "Tidy layout" — a File-menu action over the whole document, plus a pair of TB/LR buttons in `SelectionToolbar` scoped to the selection — and both were removed: on a real diagram the arrangement it produced read as scattered rather than tidy, and a one-click action that rearranges the user's whole canvas has to be clearly better than what it replaces or it is worse than nothing. `layoutDocument` stays because the importer is its only remaining caller, and there the alternative isn't the user's own arrangement — it's no coordinates at all. Don't re-expose it as a command without fixing the arrangement first; the gap is that the coordinate pass only packs each rank and centres it, so nothing pulls a node into line with its own inputs, and a multi-rank edge gets no lane reserved for it.

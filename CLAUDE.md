...
- **A swimlane is a group**, not a sixth node kind: the dock's Lane tool draws a frame pre-styled the way every layered-architecture and pipeline diagram wears them — numbered header (`01 / Lane`), dashed hairline, a wash barely there, in neutral greys that read on a light *or* dark canvas. Drawn tall it reads as a column and centres its label; drawn wide it reads as a band and labels from the left. `addLaneAfter` tiles an empty copy alongside (right of a column, below a band) with the number incremented, because a row of lanes is never built once — it clones the frame only, never its members. Group headers honour their own `textAlign`, read off the **raw** field rather than the resolved one: unset resolves to `'center'` everywhere else, and every frame saved before this was left-aligned.
...
`type: 'legend'` is the key every reference diagram carries: rows of sample + label naming the colour and line vocabulary in use. `FlowNode.legend` holds a `LegendSpec` (`{ items: LegendItem[], orientation? }`); each item is a `swatch` (a filled chip, for a node colour role) or a `line` (a rule with an arrow head, in any `lineStyle`, for an edge style). It paints no card — same invisible-hit-rect + hover-outline pattern as text and free icon objects, because in the references the legend sits bare under the drawing rather than in a box — and picks up the same exclusions: no ports, filtered out of `computeOrderedGroups`, motion-only effects.

`LegendInspector` edits the rows directly (colour, label, and a chip / solid / dashed / dotted sample toggle), plus orientation, typography and a label colour that applies to the text while each row's sample keeps its own colour.

**A row can follow a line-style class**, and that's how "Generate from diagram" works: `buildLegendItemsFromDocument` emits one row per class in `usedEdgeStyles(doc)` — the classes at least one line actually references, since vocabulary the reader never meets would mislead in a key — carrying nothing but `styleRef`. `resolveLegendItem` then fills colour, stroke pattern and label from the class at paint time (same precedence as an edge: the row's own value wins), so renaming or recolouring a class updates the legend rather than leaving it stale. Generating is a one-time action; staying in sync is not. `resolveDocumentStyles` resolves legend rows alongside edges, so `FlowNodeCard` never needs the palette passed down.

The earlier note here — that auto-generated rows would come out labelled "Line 1", "Line 2" — was true only while an edge carried a bare colour with no name. Named classes are exactly the missing names.

### Sequence diagrams

A sequence diagram is the same document as everything else — two new node kinds, one new routing value, and a restyled group frame. Nothing about it is a separate mode.

- **`type: 'lifeline'`** is a participant: a header card (`LIFELINE_HEADER_HEIGHT`) with a long dashed line hanging below it. The node's `height` is the **whole** span, header included, so dragging the bottom handle is "how far down does this participant live" — the only thing anyone wants to change about one. The dashed line is 1.5px and far too thin to grab, so `FlowNodeCard` lays a 16px-wide invisible band over it as the drag target. It keeps the standard four ports (their left/right pair lands mid-span on the line, which is where you reach for a message) but is excluded from `<NodeEffectLayer>`: its `outline.d` spans the whole node while the only thing it paints is the header, so a halo would ring several hundred pixels of empty column.
- **`type: 'activation'`** is the stretch of time a participant is busy. `FlowNode.lifelineId` binds it to its lifeline — deliberately **not** `parentId`, which means "member of a group frame" and which `onNodeDrop` clears whenever the node isn't inside one; a bar would lose its lifeline the first time it was dragged. The store's `onNodeMove` forces its x to the lifeline's centre line and carries every bar when the lifeline itself moves; `onNodeDelete` takes the bars with the lifeline. `ActivationInspector` therefore offers no X field at all — a control that silently snapped back would be a lie.
- **`routing: 'message'`** is the only route whose geometry isn't derived purely from its two endpoints. A lifeline is one vertical line spanning the whole drawing, so "which lifelines" fixes only x; the y — *when* the message happens — is the edge's own `messageY`. `buildMessagePath` insets both ends by `ACTIVATION_INSET` (half a bar's width) so an arrow lands on a bar's edge instead of vanishing underneath it, and renders `from === to` as a loop out and back rather than a zero-length line. `onConnect` stamps `routing: 'message'` plus a stacked `messageY` whenever both ends are lifelines.
- **Dragging a message moves it vertically only** — `FlowCanvas`'s `messageDrag`, wired through `AnimatedEdge`'s `onLinePointerDown`, which is passed only for message edges (on every other route a drag on the body belongs to panning). Grid snapping applies, so a column of messages lines up without hand-nudging. A self-message can't be drawn at all — `finishLink` needs two different nodes — so `LifelineInspector` has an explicit button for it, alongside "Add activation".
- **A fragment band is a group**, not a ninth node kind: `frameStyle: 'fragment'` swaps the full-width title bar for a small corner tab (`FRAGMENT_TAB_HEIGHT`, tinted with `borderColor` at 0.2 — the frame's own wash is deliberately barely-there and leaves the tab invisible). A full-width bar would cut across every lifeline running behind it. Same "a lane is a group" precedent, and `GroupInspector` exposes the Panel/Fragment toggle on any frame.
- **Lifelines and activation bars are scenery**, added to `SCENERY_NODE_TYPES` alongside frames, text, free icons and the legend. That is exactly right here rather than a compromise: in a sequence diagram the **messages** are the steps, and messages are edges, so `computeRunTimeline` already replays them in `sortOrder` order for free.
...
- **`src/lib/mermaid-import.ts`** — a hand-written parser for `flowchart`/`graph`, `sequenceDiagram` and `erDiagram`. Not mermaid's own grammar: it covers a dozen chart types this editor can't represent, so the dependency would be large and mostly rejecting input. **Unrecognised lines become warnings, never an exception** — a 90%-understood paste is far more useful than an error, and the dialog shows what was dropped *before* the import replaces the canvas.
  ...
  - `IMPORT_PAINT` carries three values per hue, not two: the pale/deep pair a filled card wears, plus an `ink` mid-tone for anything drawn straight onto the canvas (a lifeline's dashed line, an activation bar, every imported line). The pale value is a dark-canvas colour and all but vanishes on a light one at the base path's 52% stroke opacity.
  - A sequence band closes on the *last message placed*, not on `messageY` — that has already advanced to the next slot by the time `end` is read, so closing on it swallows the message after the band.
- **`src/lib/starter-diagrams.ts`** — each starter is stored as its mermaid source and built through the importer on demand, so there is one description of "a small sequence diagram" rather than a hand-built document beside it that drifts. Every starter therefore doubles as a worked example of what the importer accepts. These are **not** the template library: templates live in Firestore and an empty collection still yields an empty library. Starters ship with the app.
...
- **Creating**: the dock's Table tool arms `activeShape = 'table'` (`DrawTool = NodeShape | 'table' | 'group' | 'text' | 'icon'`); `onShapeCreate` builds a `rounded` card with `starterColumns()` and a height from `tableCardHeight(columnCount)`, not from the drag — a new table is never born with rows clipped.
...
`components/inspector/NodeInspector.tsx` is a dispatcher, not a panel: it renders `BlockInspector`, `TextInspector`, `GroupInspector`, `IconInspector` or `LegendInspector` based on `node.type`. The five kinds paint genuinely different things — a block has a body, a text object has only words, a frame has a wash and a title bar, a free icon/logo object has only a glyph, a legend has rows of samples — so each panel lists only the controls that do something for that kind. Don't add a `node.type === …` guard inside a panel; put the control in the panel it belongs to.
...
- **`LegendInspector`** — the rows themselves (colour, label, swatch/line/dashed sample), orientation, typography, label colour, opacity. No sort order: the replay skips it.

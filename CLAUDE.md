...
`type: 'line'` is a straight stroke, not attached to any node — the free-standing counterpart to an edge, built the same way text and free icon objects are: a `position`/`width`/`height` box reusing the ordinary drag/resize/snap machinery, plus `FlowNode.lineFlip` saying which diagonal of that box the visible stroke runs — unset draws top-left to bottom-right, `true` draws top-right to bottom-left. `FlowNodeCard` computes the line's two endpoints from the box and `lineFlip`, draws an invisible 16px-wide hit band along that diagonal (the box itself can be only a few pixels tall for a near-flat line, far too thin to grab), then the visible stroke through `edgeLineDash`/`edgeLineCap` off `borderStyle`, plus optional `startMarker`/`endMarker` (the same `EdgeMarkerSymbol` an edge uses) rotated to face outward at each end.

Drawing one goes through the same click-drag gesture as every other shape tool: `FlowCanvas`'s pointerup handler computes `flipped = (end.x - start.x) * (end.y - start.y) < 0` from the actual drag corners and threads it through `onShapeDrawn`/`onShapeCreate` as a fifth argument, so the drawn line matches the diagonal the user actually dragged rather than defaulting to one direction. `LineInspector` exposes the same choice explicitly as a two-way `SegmentedButtons` for adjusting it afterwards.
...
- **`LineInspector`** — geometry, a direction toggle for `lineFlip`, stroke colour/width/style, start/end `MarkerPicker`s, opacity. No title, no typography, no sort order: the replay skips it.

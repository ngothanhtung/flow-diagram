...
`type: 'legend'` is the key every reference diagram carries: rows of sample + label naming the colour and line vocabulary in use. `FlowNode.legend` holds a `LegendSpec` (`{ items: LegendItem[], orientation? }`); each item is a `swatch` (a filled chip, for a node colour role) or a `line` (a rule with an arrow head, dashed or solid, for an edge style). It paints no card — same invisible-hit-rect + hover-outline pattern as text and free icon objects, because in the references the legend sits bare under the drawing rather than in a box — and picks up the same exclusions: no ports, filtered out of `computeOrderedGroups`, motion-only effects.

`LegendInspector` edits the rows directly (colour, label, and a swatch / line / dashed sample toggle), plus orientation, typography and a label colour that applies to the text while each row's sample keeps its own colour. Rows are hand-written for now: generating them from the diagram waits on named edge styles, because today an edge carries a bare colour with no name, so anything auto-collected would come out labelled "Line 1", "Line 2" — worse than nothing.

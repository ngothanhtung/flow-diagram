...
function DrawPreview({ shape, start, current }: { shape: DrawTool; start: { x: number; y: number }; current: { x: number; y: number } }) {
  ...
  // create; the text and icon tools preview as a plain rectangle, matching
  // the box the content will be laid out / sized within.
  const previewShape: NodeShape = shape === 'table' || shape === 'logo' || shape === 'group' ? 'rounded' : shape === 'text' || shape === 'icon' ? 'rectangle' : shape;

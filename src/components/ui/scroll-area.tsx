  ...
  return (
    <ScrollAreaPrimitive.Root data-slot='scroll-area' className={cn('relative', className)} {...props}>
      <ScrollAreaPrimitive.Viewport data-slot='scroll-area-viewport' className='size-full rounded-[inherit] transition-[color,box-shadow] outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-1'>
        {/* Base UI's content box defaults to `min-width: fit-content`,
            which shrinks children to their intrinsic width; pin it to the
            viewport so panels fill the sidebar instead of drifting. */}
        <ScrollAreaPrimitive.Content data-slot='scroll-area-content' style={{ minWidth: '100%' }}>
          {children}
